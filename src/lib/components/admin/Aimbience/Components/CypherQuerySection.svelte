<script lang="ts">
	...
	function isCypherQuery(entry: any): boolean {
		...
		const hasKeywords = cypherKeywords.some(keyword => 
...
{#if isCypher}
	<div class="space-y-3">
		...
		{#if cypherQuery}
			<div class="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-lg p-4 border border-purple-200 dark:border-purple-700">
				<div class="flex items-center gap-2 mb-3">
					...
					{#if entry.query_type}
						<span class="px-2 py-1 bg-purple-100 dark:bg-purple-800/50 text-purple-700 dark:text-purple-300 rounded text-xs font-medium">
							{entry.query_type}
						</span>
					{/if}
				</div>
				<pre class="text-sm bg-white dark:bg-gray-800 p-3 rounded border overflow-auto max-h-32 text-gray-800 dark:text-gray-200 font-mono leading-relaxed">
		...
		{#if cypherResults}
			<div class="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20 rounded-lg p-4 border border-green-200 dark:border-green-700">
				<div class="flex items-center gap-2 mb-3">
					...
					<span class="px-2 py-1 bg-green-100 dark:bg-green-800/50 text-green-700 dark:text-green-300 rounded text-xs font-medium">
						{cypherResults.length} result{cypherResults.length !== 1 ? 's' : ''}
					</span>
				</div>
				<pre class="text-sm bg-white dark:bg-gray-800 p-3 rounded border overflow-auto max-h-48 text-gray-800 dark:text-gray-200 font-mono leading-relaxed">
